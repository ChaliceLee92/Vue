<template>
    ...
    >
        ...
        >
            <li
                v-for="(item, index) in rightMenu.list.filter(
                    item => item.show
                )"
                :key="index"
                @click="
                    handleRightEvent(item.type, item.data, item.node, item.vm)
                "
            >
                {{ item.name }}
            </li>
...
export default {
    ...
    watch: {
        ...
        treeRefresh(val) {
            if (this.lazy) {
                const level =
                    'node' +
                    (this.refreshLevel - 1 >= 0 ? this.refreshLevel - 1 : 0)
    ...
    methods: {
                ...
                if (data.leaf) {
                    dom = (
                        <p class="custom-tree-node">
                            <img src={require('@assets/images/doc.png')}></img>
                    ...
                    dom = (
                        <p class="custom-tree-node">
                            <img
                                src={
                                    node.expanded
                                        ? require('@/assets/images/file-open.png')
                                        : require('@/assets/images/file-close.png')
                                }
                            ></img>
                            <span style="margin-left:5px;" title={data.desc}>
                                {node.label}
        ...
        handleTreeClick(e) {
            ...
            const len = this.rightMenu.list.length
            let top
                ...
                .then(res => {
                    ...
                    if (res.success) {
                        ...
                        if (
                            ...
                            this.defaultHighLight === 0
                        ) {
                            this.$nextTick(() => {
                                this.$refs.TreeComponent.setCurrentKey(
                                    this.defaultHighLight
                                )
                            })
                        }
                        ...
                        if (
                            this.defaultClicked &&
                            (this.defaultClicked.id ||
                                this.defaultClicked.id === 0)
                        ) {
