			...
			if (this.loadType === 2) {
				if (data.leaf) {
					...
					dom = (
						<p class="custom-tree-node">
							<img
								src={node.expanded ? require('@/assets/images/file-open.png') : require('@/assets/images/file-close.png')}
							></img>
							<span style="margin-left:5px;" title={data.desc}>
								{node.label}
							</span>
				...
				.then(res => {
					...
					if (res.success) {
						...
						if (this.defaultClicked && (this.defaultClicked.id || this.defaultClicked.id === 0)) {
							...
							this.$emit('handleEvent', 'leftClick', {
								data: this.getSelectData(loadInfo.key, this.baseData, this.defaultClicked.id) || {}
							});
						}
					} else {
						this.$message({
				...
				.then(res => {
					...
					if (res.success) {
						...
						if (this.defaultClicked && (this.defaultClicked.id || this.defaultClicked.id === 0)) {
							// 页面初始化，设置默认点击项， 并将点击事件派发到父级
							this.$emit('handleEvent', 'leftClick', {
								data: this.getSelectData(levelInfo.key, this.baseData, this.defaultClicked.id) || {}
							});
						}
					} else {
						this.$message({
