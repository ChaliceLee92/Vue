<template>
	...
	>
		...
		<el-button v-if="!autoUpload" style="margin-left: 10px;" size="small" type="success" @click="submitUpload">
			开始上传
		</el-button>
...
export default {
	...
	methods: {
		...
		init() {
			// 设置token
			this.headersData = { Authorization: getToken('adminToken') };
		...
		uploadSuccess(res, file, fileList) {
			...
			this.$message({
				showClose: true,
				message: res ? '上传成功' : '上传失败',
				type: res ? 'success' : 'error',
				duration: 3500
			});

			this.$emit('handleEvent', 'upload', res);
		},
		// 文件上传失败
